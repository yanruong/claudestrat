"""
Phase 3: LightGBM + SHAP with Purged Walk-Forward CV + Optuna
- Purged walk-forward CV (5-fold, 5-day embargo)
- Optuna hyperparameter tuning (custom Score as objective)
...
OUT = '/home/user/claudestrat/output/phase3_v2'
...
df = pd.read_csv('/home/user/claudestrat/data/features_v2.csv', index_col=0, parse_dates=True)
df = df.sort_index()
...
with open('/home/user/claudestrat/data/phase3_v2_results.json', 'w') as f:
    json.dump(results, f, indent=2)

# Save last fold model + scaler for Phase 4/5
with open('/home/user/claudestrat/data/model_last_fold.pkl', 'wb') as f:
...
ax.set_title('Mean |SHAP| — Feature Importance (OOF)')
...
plt.title('SHAP Summary (OOF test sets)')
plt.tight_layout()
...
axes[0].set_title(f'OOF Equity Curve  |  Score={oof_score:.4f}  |  MaxDD=RM{oof_md:,.0f}')
axes[0].legend()
...
axes[0].set_title('Fwd Return Distribution by Signal')
axes[0].set_xlabel('Fwd Return (%)')
axes[0].legend()

prob_bins = np.linspace(0, 1, 11)
oof_df['prob_bin'] = pd.cut(oof_df['prob_long'], bins=prob_bins)
cal = oof_df.groupby('prob_bin', observed=True)['fwd_ret'].mean() * 100
cal.plot(kind='bar', ax=axes[1], color='steelblue', edgecolor='none')
axes[1].axhline(0, color='black', lw=0.8)
axes[1].set_title('Mean Fwd Return by Predicted Probability Decile')
axes[1].set_xlabel('P(Long) bin')
axes[1].set_ylabel('Mean Fwd Return (%)')
