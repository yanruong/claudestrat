"""
Phase 2b: Rebuild features adding MYR/USD cross-asset signals
New features:
...
All features still shifted by 1 day (look-ahead free).
...
# Align to FCPO trading days (forward-fill FX on non-trading days)
myr_aligned = myr_close.reindex(daily.index, method='ffill')
print(f"MYR/USD aligned: {myr_aligned.notna().sum()} / {len(daily)} days")
print(f"MYR/USD range: {myr_aligned.min():.4f} – {myr_aligned.max():.4f}")

# ── 3. FCPO feature series ────────────────────────────────────────────────────
...
# Rolling 22d correlation: MYR return vs FCPO return
myr_fcpo_corr = myr_ret_1d.rolling(22).corr(daily['ret_1d']).rename('myr_fcpo_corr')

# CPO price in USD: FCPO_close / MYR_USD (proxy for dollar-denominated CPO)
cpo_usd       = (daily['close'] / myr_aligned)
cpo_usd_ret   = np.log(cpo_usd).diff(1).rename('cpo_usd_ret_1d')
cpo_usd_z22   = ((cpo_usd - cpo_usd.rolling(22).mean()) /
                  (cpo_usd.rolling(22).std() + 1e-9)).rename('cpo_usd_z_22')

# ── 5. Build feature matrix (shift all by 1 — look-ahead free) ───────────────
...
myr_numeric  = ['myr_ret_1d','myr_ret_5d','myr_vol_ratio','myr_z_22',
                'myr_fcpo_corr','cpo_usd_ret_1d','cpo_usd_z_22']
...
myr_series = pd.DataFrame({
    ...
    'cpo_usd_ret_1d': cpo_usd_ret,
    'cpo_usd_z_22':   cpo_usd_z22,
})
...
# ── 6. VIF iterative removal ──────────────────────────────────────────────────
print("\n" + "="*60)
print("VIF — ALL NUMERIC FEATURES (pre-filter)")
print("="*60)
all_numeric = fcpo_numeric + myr_numeric
...
# MYR/USD vs FCPO price over time
...
axes[1].set_title('MYR/USD Rate (higher = weaker MYR)')
axes[1].set_ylabel('MYR per USD')

myr_fcpo_corr.reindex(feat_out.index).plot(ax=axes[2], color='dimgray', lw=0.8)
axes[2].axhline(0, color='black', lw=0.8)
axes[2].set_title('Rolling 22d Correlation: MYR/USD return vs FCPO return')
axes[2].set_ylabel('Pearson r')
plt.tight_layout()
plt.savefig(f'{OUT}/01_myrusd_vs_fcpo.png', dpi=150)
plt.close()
...
ax.set_title('Feature Correlation Matrix v2 (with MYR/USD features)')
plt.tight_layout()
plt.savefig(f'{OUT}/02_correlation_matrix.png', dpi=150)
plt.close()

# Forward return by MYR/USD quintile
fig, axes = plt.subplots(1, 3, figsize=(15, 5))
myr_feats_plot = [f for f in ['myr_ret_1d','myr_z_22','myr_fcpo_corr'] if f in feat_out.columns]
for i, col in enumerate(myr_feats_plot[:3]):
...
plt.suptitle('Mean Forward Return by MYR/USD Feature Quintile')
plt.tight_layout()
plt.savefig(f'{OUT}/03_myr_features_vs_fwdret.png', dpi=150)
plt.close()

print(f"\nAll plots saved to {OUT}/")
print("\nPhase 2b complete. Ready to re-run Phase 3 with MYR/USD features.")
